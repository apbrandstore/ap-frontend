...
export function generateOrderPDF(data: OrderPDFData): void {
  ...
  addCenteredText("AP Brand Store", yPos, 24);
  yPos += 8;
  ...
  data.items.forEach((item) => {
    ...
    if (doc.getTextWidth(displayName) > 45) {
      while (doc.getTextWidth(displayName + "...") > 45 && displayName.length > 0) {
    ...
    if (doc.getTextWidth(displayVar) > 45) {
      while (doc.getTextWidth(displayVar + "...") > 45 && displayVar.length > 0) {
  ...
  addCenteredText("Thank you for shopping with AP Brand Store!", yPos, 11);
  yPos += 6;
  addCenteredText("For any queries, please contact us.", yPos, 10);

  const safeName = data.orderRef.replace(/[^a-zA-Z0-9_-]/g, "_");
  doc.save(`APBrandStore_Order_${safeName}.pdf`);
}
